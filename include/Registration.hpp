...
public:
	 ...
	 *\param[in] VfiltSize Voxel filter size in meters
	 ...
	 * \warning the image must not be changed.
	...
	/**
		 ...
		 * \param[out] _out the resultant cloud
		 */
...
private:
	...
	/**
	 * \brief Visualise both point clouds with no registration transformation
	 */
	void view (const PCXYZRGB::ConstPtr &src
			, const PCXYZRGB::ConstPtr &tgt);

	/**
	 * \brief Hardware based normal esimation function if _downsample set to true a cloud
